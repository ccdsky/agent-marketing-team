...
**Install:** `claude plugin install agent-marketing-team`
...
Run the init skill to scaffold your working directory:

```
/agent-marketing-team:init
```

This creates `context/`, `knowledge/`, and `output/` directories with template files.
...
|------|-------------------|
| `context/voice-dna.md` | Your writing voice — sentence patterns, word choice, tone, what you'd never say. Paste 3-5 writing samples. |
| `context/icp.md` | Ideal customer profile — pain points, goals, language, where they hang out |
| `context/business-profile.md` | Products, pricing, positioning, customer results |
...
The Distribution Specialist reports using a three-layer scorecard that leads with business impact, not vanity metrics.

### The Three Layers

| Layer | What It Answers | Examples |
|-------|----------------|----------|
| **Tier 1 — Business Outcomes** | Did marketing move the needle? | Revenue, LTV, ROI, cost per acquisition, pipeline contribution |
| **Tier 2 — Demand Signals** | Is marketing building real momentum? | Conversion quality (close rate, deal size), conversion velocity (time-to-close), brand search growth, share of voice, net-new vs. existing contacts |
| **Tier 3 — Funnel Diagnostics** | Why did the numbers move? | Traffic, opt-in rates, open rates, click rates, bounce rates |

Every report — daily, weekly, and post-campaign — leads with Tier 1. Tier 3 metrics explain *why* something changed but never headline a report.

### Incrementality

Every weekly report includes an incrementality assessment: what percentage of conversions came from net-new contacts vs. people already in the CRM or email list. This answers the question executives actually care about: *"Did marketing create new revenue, or just take credit for demand that already existed?"*

If >70% of conversions are from existing contacts, the agent flags this — the campaign may be capturing demand rather than creating it.

### How It Works in Practice

The agent can't query analytics tools directly. Instead, it asks the user to paste metrics — but the *questions it asks* are structured around the three layers. When the agent asks for revenue per lead instead of page views, it changes what the user pays attention to.

Pre-launch, the agent sets up a three-layer metrics framework with the Campaign Lead. Post-launch, every report follows the same structure: outcomes first, demand signals second, diagnostics third.
...
**Used in:** All content skills — `/landing-page`, `/email-sequence`, `/blog-post`, `/newsletter`, `/social-post`, `/lead-magnet`, `/repurpose`
...
Post-Launch:
  Outcomes-first reporting   → revenue, LTV, ROI (Tier 1)
  Demand signal tracking     → conversion quality, velocity, brand search (Tier 2)
  Incrementality assessment  → net-new vs. existing demand
  Funnel diagnostics         → traffic, open rates, clicks (Tier 3, supporting)
```
