...
#### 3. Body (one idea per email, voice-matched)

One CTA per email. Escalate CTA intensity across the sequence: soft (reply, read) → medium (download, watch) → hard (buy, book a call).

### Step 3: Verify Arc and Voice

Read full sequence end-to-end. Verify CTA escalation is gradual and voice matches `voice-dna.md` throughout.
